...
#pragma optimize("", off)
#endif
NWMServer::NWMServer(const std::shared_ptr<SVNWMUDPConnection> &udp, const std::shared_ptr<SVNWMTCPConnection> &tcp) : NWManagerBase(udp, tcp), m_maxClients(-1)
{
	auto packetHandle = std::bind(
	  [](NWMServer *sv, const NWMEndpoint &ep, NWMIOBase *io, unsigned int id, NetPacket &readPacket) {
	...
	auto clientHandle = [](NWMServer *sv, NWMSession *session) {
		...
		if(cl->IsReady()) {
			...
			sv->QueueEvent([sv, ptrCl]() { sv->OnClientConnected(ptrCl.get()); });
		}
	};
	if(udp != nullptr) {
		//udp->SetServer(this);
