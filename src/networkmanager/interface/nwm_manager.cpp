...
nwm::Error::Error(int err, const std::string &msg) : m_value(err), m_message(msg) {}
...
nwm::impl::ManagerBase::ManagerBase(const std::shared_ptr<NWMUDPConnection> &udp, const std::shared_ptr<NWMTCPConnection> &tcp) : m_conUDP(udp), m_conTCP(tcp) {}
...
void nwm::impl::ManagerBase::Close()
{
	if(m_bActive == false)
		return;
	ScheduleAsyncEvent([this]() { CloseAsync(); });
	m_bActive = false;
}
