...
#pragma optimize("",off)
#endif
UDPMessageDispatcher::Message::Message(nwm::IOService &ioService,std::function<void(nwm::ErrorCode,Message*)> _callback)
	: socket(std::make_unique<nwm::UDPSocket>(ioService,boost::asio::ip::udp::v4())),callback(_callback),m_bComplete(false),external_socket(nullptr)
{}

UDPMessageDispatcher::Message::Message(nwm::UDPSocket &_socket,std::function<void(nwm::ErrorCode,Message*)> _callback)
...
void UDPMessageDispatcher::Message::Receive(unsigned int size,std::function<void(nwm::ErrorCode,DataStream)> cb)
{
	...
	nwm::cast_socket(*socket)->async_receive(
...
UDPMessageDispatcher::DispatchInfo::DispatchInfo(DataStream &_data,const std::string &_ip,unsigned short _port,std::function<void(nwm::ErrorCode,Message*)> _callback)
	: data(_data),ip(_ip),port(_port),callback(_callback),valid_endpoint(false),socket(nullptr)
{}

UDPMessageDispatcher::DispatchInfo::DispatchInfo(DataStream &_data,const nwm::UDPEndpoint &_ep,std::function<void(nwm::ErrorCode,UDPMessageDispatcher::Message*)> _callback)
