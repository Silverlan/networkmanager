...
void CLNWMTCPConnection::Connect(std::string serverIp, unsigned short serverPort)
{
	...
	boost::asio::async_connect(*nwm::cast_socket(*socket), results, [this](const boost::system::error_code &ec, const boost::asio::ip::tcp::endpoint &connected_ep) { HandleOnConnect(ec); });
}
